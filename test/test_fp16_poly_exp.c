...
/* Test fp16_poly_exp with values in [-ln(2), ln(2)] range */
void test_fp16_poly_exp_in_range(test_stats_t *stats)
{
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++)
    {
        ...
        /* Convert to fp16, apply poly_exp, convert back */
...
/* Test fp16_poly_exp with positive values [0, ln(2)] */
void test_fp16_poly_exp_positive_extended(test_stats_t *stats)
{
    struct timespec start, end;
    double total_error = 0.0;
    double max_error = 0.0;
...
/* Test fp16_poly_exp with negative values [-ln(2), 0] */
void test_fp16_poly_exp_negative_extended(test_stats_t *stats)
{
    struct timespec start, end;
    double total_error = 0.0;
    double max_error = 0.0;
...
/* Test fp16_poly_exp with mixed positive and negative values */
void test_fp16_poly_exp_mixed(test_stats_t *stats)
{
    struct timespec start, end;
    double total_error = 0.0;
    double max_error = 0.0;
...
/* Test special cases */
void test_fp16_poly_exp_special_cases(void)
{
    int passed = 0;
    int failed = 0;
...
int main(void)
{
    ...
    printf("==========================================================\n");
    printf("FP16 Polynomial Exponential Function (fp16_poly_exp) Test Suite\n");
    printf("==========================================================\n");
    printf("Testing with %d iterations per test\n", NUM_ITERATIONS);
    printf("Test Range: [%.6f, %.6f] ([-ln(2), ln(2)])\n",
           TEST_RANGE_MIN, TEST_RANGE_MAX);
    printf("==========================================================\n");
    printf("NOTE: Testing polynomial approximation of exp(x).\n");
    printf("      3rd-degree polynomial: exp(x) ≈ 1 + x + x²/2 + x³/6\n");
    printf("==========================================================\n");

    /* Test 1: Values in full range */
    test_fp16_poly_exp_in_range(&stats);
    print_results("Test 1: fp16_poly_exp(x) for x in [-ln(2), ln(2)]", &stats, EPSILON_HIGH);

    /* Test 2: Positive values [0, ln(2)] */
    test_fp16_poly_exp_positive_extended(&stats);
    print_results("Test 2: fp16_poly_exp(x) for x in [0, ln(2)]", &stats, EPSILON_HIGH);

    /* Test 3: Negative values [-ln(2), 0] */
    test_fp16_poly_exp_negative_extended(&stats);
    print_results("Test 3: fp16_poly_exp(x) for x in [-ln(2), 0]", &stats, EPSILON_HIGH);

    /* Test 4: Mixed positive and negative values */
    test_fp16_poly_exp_mixed(&stats);
    print_results("Test 4: fp16_poly_exp(x) for x in [-ln(2), ln(2)] (full range)", &stats, EPSILON_HIGH);

    /* Test 5: Special cases */
    test_fp16_poly_exp_special_cases();
    ...
    printf("==========================================================\n");
    printf("NOTE: Polynomial approximation provides fast computation\n");
    printf("      with reasonable accuracy for small input values.\n");
    printf("      Accuracy degrades for larger |x| values.\n");
    printf("==========================================================\n");
